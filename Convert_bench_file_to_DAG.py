...
class dag:
    ...
    def importBench(self, filename):
        ...
        if not data:
            raise ValueError(f"Empty benchmark file: {filename}")

        # 过滤空行和注释
        data = [line for line in data if line and not line.startswith('#')]
        
        if not data:
            raise ValueError(f"No valid content in benchmark file: {filename}")

        print(f"Processing {len(data)} lines from {filename}")  # Debug info
            ...
            for end_vertex in end_vertices:
                if (end_vertex != '0'):
                    if (end_vertex not in input_vertices):
                        self.edges.append(
                            (start_vertex, int(end_vertex)-Ninputs-1))
            linenr += 1

        self.edges = np.array(self.edges)
        # 修改这里:如果edges非空则用max+1,否则使用0
        if len(self.edges) > 0:
        ...
        # Add debug info
        print(f"Parsed DAG stats:")
        print(f"- Vertices: {self.n}")
        print(f"- Edges: {len(self.edges)}")
        print(f"- Output vertices: {len(self.output_vertices)}")


# EXAMPLE to import .bench file
#
# filename = "benchmarks/ISCAS85XMG/c7552.bench"
# filename = "benchmarks/ISCAS85/c17.bench"
# dag = dag(filename)
...
def benchToDAG(filename, rename_vertices=True, print_progress=False):
    ...
    for linenr in range(len(data)):  # check input file line by line
        if (data[linenr][:5] == 'INPUT'):
            ...
            if (data[linenr][:1] == 'G' or data[linenr][:1] == 'I' or data[linenr][:1] == 'g'):
                ...
                end_vertices = split[1][start +
                                        1:end].split(", ")  # inputs of gate
    ...
    vertices = np.unique(vertices, return_inverse=True)
    ...
    if (rename_vertices):
        ...
        for j, x in enumerate(dictionary):
            edges[edges == x] = j
        # convert array type from strings to integers
        edges = np.array(edges, dtype=int).reshape([nr_of_edges, 2])
        ...
        # convert array type from strings to integers
        input_vertices = np.array(input_vertices, dtype=int)
        ...
        output_vertices = np.array(
            output_vertices, dtype=int)  # convert array type from strings to integers
